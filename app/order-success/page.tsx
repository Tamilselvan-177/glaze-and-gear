  ...
  return (
    <div className="max-w-2xl w-full">
      ...
      <div className="text-center mb-10">
        ...
        <h1 className="text-4xl font-serif font-black text-gray-900 mb-3">Order Confirmed!</h1>
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-[#98202E]/5 border border-[#98202E]/10 overflow-hidden">
          <div className="px-8 py-5 border-b border-gray-100 flex justify-between items-center">
            <div>
              ...
              <p className="font-mono font-black text-gray-900">#{order.id.slice(-8).toUpperCase()}</p>
            </div>
            <div className="text-right">
              <p className="text-xs font-bold uppercase tracking-widest text-gray-400">Payment</p>
              <span className={`text-xs font-black uppercase px-3 py-1 rounded-full ${method === 'COD' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`}>
          ...
          <div className="divide-y divide-gray-50">
            {order.items?.map((item: any) => (
              <div key={item.id} className="flex gap-4 items-center px-8 py-5">
                ...
                <p className="font-black text-[#98202E]">₹{(item.price * item.quantity).toLocaleString()}</p>
              </div>
            ))}
          </div>
          ...
          <div className="px-8 py-6 bg-gray-50 border-t border-gray-100 space-y-3">
            <div className="flex justify-between items-center">
              <span className="font-bold text-gray-500 text-sm">Order Total</span>
              <span className="text-2xl font-black text-[#98202E]">₹{order.totalAmount?.toLocaleString()}</span>
            </div>
            {order.shippingAddress && (
              <div className="pt-3 border-t border-gray-200">
                <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-1">Shipping To</p>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mt-8">
        <Link href="/account/orders" className="flex-1 text-center bg-[#98202E] text-white py-4 px-8 rounded-xl font-bold uppercase tracking-widest hover:bg-[#7a1a25] transition-all shadow-xl shadow-[#98202E]/20">
          Track My Orders
        </Link>
        <Link href="/products" className="flex-1 text-center border-2 border-[#98202E] text-[#98202E] py-4 px-8 rounded-xl font-bold uppercase tracking-widest hover:bg-[#98202E]/5 transition-all">
          Continue Shopping
        </Link>
      </div>
...
export default function OrderSuccessPage() {
  return (
    <div className="min-h-screen pt-[120px] bg-[#F9EAEA]/30 flex items-center justify-center px-[5%] py-12">
      <Suspense fallback={<div className="text-[#98202E]">Loading...</div>}>
        <OrderSuccessContent />
      </Suspense>
    </div>
  );
