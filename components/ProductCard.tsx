    ...
    <Link href={`/products/${product.slug || product.id}`} className={`no-underline block h-full ${isOutOfStock ? "opacity-75" : ""}`}>
      <div className="bg-white border border-[#98202E]/10 p-4 sm:p-6 rounded-[20px] transition-all duration-500 hover:-translate-y-[10px] hover:shadow-[0_30px_60px_rgba(152,32,46,0.15)] cursor-pointer text-left relative h-full flex flex-col group">
        ...
        <button 
          ...
          className="absolute top-6 right-6 z-10 w-10 h-10 bg-white/90 backdrop-blur-md rounded-full shadow-[0_4px_12px_rgba(0,0,0,0.1)] flex items-center justify-center transition-all duration-300 hover:scale-110 hover:shadow-[0_8px_16px_rgba(0,0,0,0.15)] group/btn"
        ...
        >
          <svg 
            ...
            className={`w-5 h-5 transition-all duration-300 ${inWishlist ? "text-[#98202E] scale-110" : "text-gray-400 group-hover/btn:text-[#98202E] group-hover/btn:scale-110"}`}
          >
        ...
        <div className="w-full aspect-square rounded-xl mb-4 sm:mb-6 overflow-hidden relative">
          ...
          {isOutOfStock && (
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
              <span className="bg-[#98202E] text-white font-bold text-xs uppercase tracking-widest px-4 py-2 rounded">
                Out of Stock
              </span>
            </div>
          )}
        ...
        <div className="flex-1 flex flex-col">
          ...
          <h3 className="text-xl sm:text-2xl font-serif text-[#98202E] mb-2 line-clamp-2 min-h-[3.5rem] sm:min-h-[4rem] leading-tight">{product.name}</h3>
